...
class CSVSourceConfig(BaseModel):
    ...
    path: str = Field(..., description="Путь к CSV файлу (локальный, HTTP, S3, FTP)")
    ...
    # S3 параметры
    aws_access_key: str | None = Field(default=None, description="AWS Access Key")
    aws_secret_key: str | None = Field(default=None, description="AWS Secret Key")
...
class CSVExtractConfig(ComponentConfig):
    ...
    source_config: CSVSourceConfig = Field(..., description="Конфигурация источника")
    ...
    skip_rows: int = Field(default=0, ge=0, description="Пропустить строк сверху")
