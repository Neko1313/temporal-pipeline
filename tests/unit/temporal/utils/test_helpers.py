...
from tests.fixtures.yaml_loader.interfaces import (
    FactoryStageConfig,
    FactoryStageExecutionResult,
)


def test_prepare_input_data(
    factory_stage_config: FactoryStageConfig,
    factory_stage_execution_result: FactoryStageExecutionResult,
) -> None:
    ...
    result_prepare_input_data = prepare_input_data(
        stage_config, {stage_name: stage_execution_result}
    )
    assert result_prepare_input_data is not None


def test_prepare_input_data_not_depends_on(
    factory_stage_config: FactoryStageConfig,
    factory_stage_execution_result: FactoryStageExecutionResult,
) -> None:
    stage_name = "test_name"
...
def test_prepare_input_data_not_depends(
    factory_stage_config: FactoryStageConfig,
) -> None:
    stage_config = factory_stage_config.build()
